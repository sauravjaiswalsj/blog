...
- Given input (X) and output labels (Y), learn a mapping f: X -> Y
...
- **Clustering**: group similar items; modelf: X -> {1, ..., K}, where K is the number of classes is also unknown.
- **Dimensionality reduction**: map high‑dimensional data (e.g. images) to low‑dimensional coordinates (F({image}) = (x, y)) that capture main variability (e.g. pose, expression).
...
* If `y_i = 1`, the loss becomes:
  ...
  * If the model predicts `y_hat_i = 0.9`
    → loss = `-log(0.9)` (small)

  * If the model predicts `y_hat_i = 0.01`
    → loss = `-log(0.01)` (huge)
...
* If `y_i = 0`, the loss becomes:
  ...
  * If the model predicts `y_hat_i = 0.1`
    → loss = `-log(0.9)` (small)

  * If the model predicts `y_hat_i = 0.99`
    → loss = `-log(0.01)` (huge)
